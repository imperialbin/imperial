  ...
  switch (true) {
    ...
    case e.key === "t" && e.altKey:
      if (location.pathname === "/") {
        window.open(
          `https://twitter.com/intent/tweet?text=I%20use%20Imperialbin!%20An%20advanced%20and%20feature%20rich%20pastebin,%20start%20using%20it%20today%20at%20https://www.imperialbin.com/!`,
          "_blank"
        );
      } else {
        window.open(
          `https://twitter.com/intent/tweet?text=Here%20is%20a%20paste%20I%20made%20on%20ImperialBin%20https://www.imperialbin.com${location.pathname}`,
          "_blank"
        );
      }
      break;
...
function saveSettings() {
  ...
  if (deleteCheck && $(".error").length === 0) {
    $("#messages").append(
      '<li class="message error"><i class="fas fa-info-circle" style="padding-right: 4px;"></i> Instant delete is on!</li>'
    );
  } else if (!deleteCheck) {
    $(".error").remove();
  }
...
function toggleAddUser() {
  ...
  if (getUsers !== null && getUsers !== "") {
    for (let users = 0; users < getUsers.split(",").length; users++) {
    ...
    .then((json) => {
      if (json.status === "success") {
        if (instantDelete === "true") {
          ...
          const lines = $("#codeThing").val().split("\n");
