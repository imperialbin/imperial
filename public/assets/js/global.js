...
function editPaste() {
  $(".editButton").remove();
  $("#edit").append(
    '<button id="icon" class="editButton editButtonActive" onclick="postEditPaste(); emitEditPost()"><i class="fas fa-check"></i></button>'
  );
  $("#box").css("display", "none");

  const $codeThing = $("#codeThing");
      ...
      .then((json) => {
        if (json.status === "success") {
          ...
          if ($(".message, .success").length === 0) {
            $("#messages").append(
              '<li class="message success"><i class="fas fa-check" style="padding-right: 9px;"></i> Edited post!</li>'
            );
          }
